...
//	    url = url.replace('{start}', start).replace('{length}', length);
      ...
      filterSystems(systems, params){
          if(params.name){
           systems = systems.filter(function (system) { return system['name'] === params.name;})
          }
          if(params.namespace){
           systems = systems.filter(function (system) { return system['namespace'] === params.namespace;})
          }
          if(params.version){
           systems = systems.filter(function (system) { return system['version'] === params.version;})
          }
          return systems;
      }
