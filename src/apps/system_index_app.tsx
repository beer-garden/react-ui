...
class SystemsApp extends Component<MyProps, MyState> {
    ...
    updateData() {
        ...
        let newData = this.systems.slice(state.page * state.rowsPerPage, (state.page * state.rowsPerPage + state.rowsPerPage));
    ...
    formatData(data: any[]) {
        ...
        for (let i in data) {
            for (let tableKey in this.state.tableKeys) {
                ...
                if (this.state.tableKeys[tableKey] === "commands" || this.state.tableKeys[tableKey] === "instances") {
                    tempData[i][this.state.tableKeys[tableKey]] = data[i][this.state.tableKeys[tableKey]].length;
                } else if (this.state.tableKeys[tableKey] === "") {
                    tempData[i][this.state.tableKeys[tableKey]] = this.exploreButton(data[i]);
                } else {
