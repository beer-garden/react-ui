...
class GardenViewApp extends Component<MyProps, MyState> {
  ...
  updateData() {
    ...
    let newData = this.system_data.slice(
  ...
  successCallback(response: any) {
    ...
    let state = this.state;
    let data = this.system_data.slice(
      state.page * state.rowsPerPage,
      state.page * state.rowsPerPage + state.rowsPerPage
    );
    ...
    this.setState({
      data: data,
      totalItems: this.garden.systems.length,
      dataForm: this.initialModel,
    });
  }
  ...
  getConfigSetup() {
    if (this.garden.connection_type === "LOCAL") {
      ...
      return (
        <GardenForm self={this} schema={this.schema} uischema={this.uischema} />
      );
    }
  }
          ...
          <Grid item>
            <Typography style={{ flex: 1 }}>
              <Button
                variant="contained"
                color="primary"
                onClick={() => GardenService.syncGarden(this.garden_name)}
              >
                Sync
              </Button>
            </Typography>
