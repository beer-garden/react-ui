    ...
    class GardenViewApp extends Component {
      ...
      getConfigSetup(){
        if(this.garden.connection_type==="LOCAL"){
            return(
                <Alert severity="info">{"Since this is the local Garden it's not possible to modify connection information"}</Alert>
            );
        } else {
            return(<GardenForm self={this} schema={this.schema} uischema={this.uischema} initialModel={this.initialModel} />);
        }
      }
          ...
          >
            ...
            <Grid item>
                <Typography style={{ flex: 1 }}>
                    <Button variant="contained" color="primary" onClick={() => GardenService.syncGarden(this.garden_name)} >Sync</Button>
                </Typography>
            </Grid>
          </Grid>
          <Divider />
