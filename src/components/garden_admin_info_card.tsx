...
const GardenInfoCard: FC<GardenInfoCardProps> = ({garden}: GardenInfoCardProps) => {
    ...
    let self = {
        state: {
            data: [["Name", garden.name], ["Status", garden.status], ["Known Namespaces", getNamespaceList(garden)], ["Systems", garden.systems.length]],
            tableKeys: [0, 1]
        }
    }
        ...
        <Box width={1 / 3} pb={1}>
            <Card className={classes.root}>
                <AppBar color="inherit" style={{background: 'lightgray'}} position="static">
                    <Toolbar>
                        <Typography variant="h6" color="inherit">Garden Info</Typography>
                    </Toolbar>
                </AppBar>
                <CardContent>
