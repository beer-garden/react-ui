...
//          self.setState({redirect: (<Redirect push to="/requests" />)})
        ...
        function makeRequest(self){
            if(self.state.errors.length > 0){
                ...
                return (
                    <Button onClick={() => GardenService.updateGardenConfig(GardenService.formToServerModel(self.garden, self.state.dataForm))} variant="contained" color="primary">
                            Save Configurations
                    </Button>
                )
            }
        ...
        <div>
            <JsonForms
              ...
              cells={materialCells}
              onChange={({ data, errors }) => {self.setState({dataForm: data, errors: errors});}}
            />
            {makeRequest(self)}
